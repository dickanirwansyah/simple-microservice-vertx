...
public class HelloConsumerMicroservice extends AbstractVerticle {
    ...
    private void invokeMyFirstMicroservice(RoutingContext rc){
        HttpRequest<JsonObject> request1 = client
                .get(8080, "localhost", "/dicka")
                .as(BodyCodec.jsonObject());

       HttpRequest<JsonObject> request2 = client
               .get(8080,"localhost", "/khannedy")
               .as(BodyCodec.jsonObject());
        ...
        Single.zip(s1, s2, (dicka, khannedy) -> {
            return new JsonObject()
                    .put("dicka", dicka.getString("message"))
                    .put("khannedy", khannedy.getString("message"));
        }).subscribe(
                result -> rc.response()
